...
- 学習行動イベントを時系列で保持（例: `chat_generated`, `audio_played`, `bookmarked`, `unbookmarked`, `opened_bookmarks`）
- 将来拡張のため柔軟な `metadata`（jsonb）を許容
- ユーザー毎の参照/登録のみ許可（RLS）
...
CREATE INDEX IF NOT EXISTS idx_study_history_user_created ON public.study_history (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_study_history_event_type ON public.study_history (event_type);

-- RLS（自分の履歴のみ参照・登録可能）
ALTER TABLE public.study_history ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Read own history" ON public.study_history;
CREATE POLICY "Read own history"
ON public.study_history FOR SELECT
USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Insert own history" ON public.study_history;
CREATE POLICY "Insert own history"
ON public.study_history FOR INSERT
WITH CHECK (user_id = auth.uid());
```

4.7 可視化/統計 用 ビュー / マテビュー

方針:

- 生ログは `public.study_history` に集約
- 可視化/統計は `analytics` スキーマにビュー/マテビューを配置
- マテビューは Supabase のスケジューラ（cron）で定期更新（例: 5 分〜1 時間）
...
select user_id,
       ...
       date_trunc('week', min(created_at)) as first_active_week
from public.study_history
group by 1;

-- 3) 週次コホート × 週次継続（例: マテビュー）
-- 実運用では期間制限や最新週のみ再計算するなど最適化を行う
